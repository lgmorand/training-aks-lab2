...
2- For the lab n°2, we need one subscription where users don't need access to. But this subscription must have a pre-existing mutualized AKS cluster and an attached ACR. They are gonna to use one mutualized SPN to deploy inside the ACR and the cluster. Users **must** have Docker for windows (or Linux) on their computer
